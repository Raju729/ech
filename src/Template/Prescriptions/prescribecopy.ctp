...
<script>
    $("#prescrip_form").submit(function(e) {
        e.preventDefault();
        var id = $("#pres_id").attr('pres_id');
//        alert(id);
//        $('#pleaseWaitDialog').modal();
        var data = $("#prescrip_form").serialize();
        var url2 =$('meta[name="redirect"]').attr('url');
        var url =$('meta[name="baseurl"]').attr('url');
        url = url+"prescriptions/ajax?op=save_prescription&id="+id;
        console.log(url);
        console.log(data);
        $.ajax({
            url:url,
            ...
            success: function(obj){
                console.log(obj);
                if(obj.status == 'success'){
                    FlashStatus(obj.status,"this patient is successfully presscribed");
//                    console.log(url2);
                    $( location ).attr("href",'/prescriptions/view/'+obj.data);
